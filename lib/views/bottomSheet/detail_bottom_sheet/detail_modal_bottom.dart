  ...
  return Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
    ...
    genericTextButton(
      ...
      func: () {
        chatBloc.add(
          ChatSendButtonEvent(
              havingNewContent: true, content: "$stringLookUp ${entry.word}"),
        );
        Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) =>
                  AIChating(appBloc: appBloc, chatBloc: chatBloc),
            ));
      },
    ),
    const Spacer(),
    genericTextButton(
      ...
      sized: textButtonSize[Size.medium]!,
      func: () {
        appBloc.add(ShareButtonTappedEvent(entry: entry));
      },
    ),
  ]);
}
...
Widget contentDetail(
    {required DictionaryEntry entry, required BuildContext context}) {
  return ListView(padding: const EdgeInsets.only(top: 15), children: [
    Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...entry.meanings.map((e) => getContent(e, context)),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
...
Widget getContent(Meaning meaning, BuildContext context) {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Row(
