  ...
  void searchProcess() {
    if (textController.text.length > 10 ||
      ...
      widget.chatBloc.add(ChatSendButtonEvent(
          havingNewContent: true, content: textController.text));
      textController.clear();
      widget.appBloc.add(const SearchingEvent(value: ''));
    } else {
      widget.chatBloc.add(ChatSendButtonEvent(
          havingNewContent: true,
          content: "\"Look up this \" ${textController.text}"));
      textController.clear();
      widget.appBloc.add(const SearchingEvent(value: ''));
    }

    Navigator.of(context).push(MaterialPageRoute(
      builder: (context) => AIChating(
          ...
          suffixIcon: textController.text.isEmpty
              ? IconButton(
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(
                      builder: (context) => AIChating(
                          chatBloc: widget.chatBloc, appBloc: widget.appBloc),
                    ));
                  },
                  icon: const Icon(
                    Icons.chat_bubble,
